...
,ga_content_group AS ga_content_group
,first_value(pagepath) over (partition by lpv_page_type, ga_content_group) as url_example
,SUM(Sessions1) over (partition by lpv_page_type, ga_content_group) AS Sessions1
,SUM(Sessions2) over (partition by lpv_page_type, ga_content_group) AS Sessions2
from
(select
CASE
WHEN REGEXP_MATCH(LOWER(hits.page.pagePath), r'^/(\?|$)') -- checked
THEN 'Homepage'
WHEN REGEXP_MATCH(LOWER(hits.page.pagePath), r'^/account/login') -- checked; need to add logic for e.g. /messages/967870/login
THEN 'Account_Login' 
...
WHEN REGEXP_MATCH(LOWER(hits.page.pagePath), r'^/account/settings') -- this is the only one
...
THEN 'Support' 
WHEN REGEXP_MATCH(LOWER(hits.page.pagePath), r'^/find-a-lawyer(\?|/|$)') -- added a slash for variations ending in / or /all-practice-areas
...
WHEN REGEXP_MATCH(LOWER(hits.page.pagePath), r'^/attorneys/*.*-[0-9]+\.html') -- this looks good
THEN 'Attorney_Profile' 
WHEN REGEXP_MATCH(LOWER(hits.page.pagePath), r'^/attorneys/*.*/reviews\.html') -- good
