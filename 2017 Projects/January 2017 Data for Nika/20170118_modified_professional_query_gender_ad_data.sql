  ...
  FROM (SELECT pd.*						
        ...
        FROM (SELECT pf.PROFESSIONAL_ID						
              ...
              THEN NULL						
					...
					,professional_claim_method_name AS claim_method
...
,recent_ads AS (
SELECT professional_id
,MIN(year_month_begin_date) AS first_ad_month
,MAX(year_month_begin_date) AS most_recent_ad_month
FROM tmp_data_dm.sr_prr_sub_bills
WHERE has_ads_during_month = 'Y'
GROUP BY professional_id
)
...
,ra.first_ad_month
,ra.most_recent_ad_month
,ca.first_cancellation_month
,CASE
	WHEN ra.most_recent_ad_month IS NULL AND fca.first_ad_run_date IS NOT NULL
		THEN 'Discrepancy'
	WHEN ra.most_recent_ad_month IS NULL
		THEN 'Non-Advertiser'
	WHEN ca.first_cancellation_month IS NULL
		THEN 'Current or Past Advertiser'
	WHEN ca.first_cancellation_month < ra.most_recent_ad_month
		THEN 'Churned & Returned'
	WHEN ca.first_cancellation_month >= ra.most_recent_ad_month
		THEN 'Current or Past Advertiser'-- 'Churned & Burned'
	ELSE 'WTF'
	END Advertising_Status
--,c2.company_size_3						
FROM pfad gt						
	LEFT JOIN license_discipline ld					
	...
	LEFT JOIN recent_ads ra
		ON ra.professional_id = gt.professional_id
	LEFT JOIN churned_advertisers ca
		ON ca.professional_id = gt.professional_id
