...
DROP TABLE IF EXISTS tmp_data_dm.sr_prr_sub_price ;
CREATE TABLE tmp_data_dm.sr_prr_sub_price AS
SELECT DISTINCT
   subscription_id
  ,full_price
FROM   (
...
DROP TABLE IF EXISTS tmp_data_dm.sr_prr_sub_bills ;
CREATE TABLE tmp_data_dm.sr_prr_sub_bills AS
SELECT
   uns.customer_id
  ,uns.professional_id
  ,uns.year_month
...
FROM
  ...
  UNION ALL
      ...
      FROM         dm.order_line_accumulation_fact olaf
        ...
        LEFT OUTER JOIN tmp_data_dm.sr_prr_sub_price mrr
                ON olaf.product_subscription_id = mrr.subscription_id
