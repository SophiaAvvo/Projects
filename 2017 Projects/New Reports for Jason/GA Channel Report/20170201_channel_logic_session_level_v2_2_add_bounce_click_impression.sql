...
,z.Most_Recent_Date AS Most_Recent_Date
...
,COALESCE(y.LPV_Page_Type, 'Unknown') AS LPV_Page_Type
,CASE
...
,SUM(SessionCount) AS Sessions
/*,EXACT_COUNT_DISTINCT(x.session_id) AS Sessions_Count */
,EXACT_COUNT_DISTINCT(x.unique_visitor_id) AS Unique_Visitors
,SUM(Northstar_Engagement_Flag) AS Sessions_with_Northstar_Action
,SUM(Email_and_Web_Contacts) AS Email_and_Web_Contacts
,EXACT_COUNT_DISTINCT(CASE
WHEN Email_and_Web_Contacts > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Email_Web_Contacts
,SUM(ClicksToLawyerWebsite) AS ClicksToLawyerWebsite
,EXACT_COUNT_DISTINCT(CASE
WHEN ClicksToLawyerWebsite > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Website_Clicks
,SUM(AskedQuesion_Flag) AS Sessions_With_Question_Asked
,SUM(NumberQuestionsAsked_CM) AS NumberQuestionsAsked_CM
,SUM(NumberQuestionsAnswered_Goal) AS Sessions_With_Question_Answered
,SUM(MessagesSentToLawyers) AS MessagesSentToLawyers
,EXACT_COUNT_DISTINCT(CASE
WHEN MessagesSentToLawyers > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Messages_Sent
,EXACT_COUNT_DISTINCT(CASE
WHEN NumberOfReviews > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Reviews
,SUM(NumberOfReviews) AS NumberOfReviews
,EXACT_COUNT_DISTINCT(CASE
WHEN NumberOfClaims > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Claims
,SUM(NumberOfClaims) AS NumberOfClaims
,EXACT_COUNT_DISTINCT(CASE
WHEN NumberOfEndorsements > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Endorsements
,SUM(NumberOfEndorsements) AS NumberOfEndorsements
,SUM(Registration_Flag) AS Sessions_with_Registration
,SUM(NumberOfRegistrations_CM) AS NumberOfRegistrations_CM
,SUM(Forms_Flag) AS Sessions_with_Forms
,SUM(Purchases_Flag) AS Sessions_with_Purchase
,EXACT_COUNT_DISTINCT(CASE
WHEN TotalEmailSubscriptions > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Email_Subscription
,SUM(TotalEmailSubscriptions) AS TotalEmailSubscriptions
,EXACT_COUNT_DISTINCT(CASE
WHEN OnsiteEmailSubscriptions > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Onsite_Email_Subscription
,SUM(OnsiteEmailSubscriptions) AS OnsiteEmailSubscriptions
,EXACT_COUNT_DISTINCT(CASE
WHEN OffsiteEmailSubscriptions > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Offsite_Email_Subscription
,SUM(OffsiteEmailSubscriptions) AS OffsiteEmailSubscriptions
,EXACT_COUNT_DISTINCT(CASE
WHEN Services_Purchases_Advisor > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Advisor_Purchase
,SUM(Services_Purchases_Advisor) AS Services_Purchases_Advisor
,EXACT_COUNT_DISTINCT(CASE
WHEN Services_Purchases_Document_Review > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Document_Review_Purchase
,SUM(Services_Purchases_Document_Review) AS Services_Purchases_Document_Review
,EXACT_COUNT_DISTINCT(CASE
WHEN Services_Purchases_Offline > 0
THEN x.session_id
ELSE NULL
END) AS Sessions_with_Offline_Purchase
,SUM(Services_Purchases_Offline) AS Services_Purchases_Offline
,SUM(Services_Purchases_Advisor) + SUM(Services_Purchases_Document_Review) + SUM(Services_Purchases_Offline) AS Services_Purchases_Total
,SUM(Purchases_Flag_Old_Test) AS Sessions_with_Purchase_Old_Logic
,SUM(Mobile_Calls_To_NonAdvertisers) AS Mobile_Call_Link_to_NonAdvertisers
,SUM(Messages_To_NonAdvertisers) AS Messages_To_NonAdvertisers
,SUM(Mobile_Calls_To_Advertisers) AS Clicks_on_Call_Link_to_Advertisers
,SUM(Messages_To_Advertisers) AS Messages_To_Advertisers
,SUM(Mobile_Calls_To_Advertisers) + SUM(Mobile_Calls_To_NonAdvertisers) AS Clicks_on_Call_Link_Total
FROM (
SELECT Date
,CONCAT(fullVisitorId,string(VisitId)) AS session_id
,fullVisitorID AS unique_visitor_id
,trafficSource.campaign AS Campaign
,trafficSource.source AS Source
,trafficSource.medium AS Medium
,device.deviceCategory AS device_type
,totals.visits SessionCount
/* New as of 02/01 */
,totals.bounces Bounce_Flag
,SUM(CASE
	WHEN LOWER(hits.eventInfo.eventAction) = 'show ad'
	AND LOWER(hits.eventInfo.eventCategory) = 'advertising'
	AND LOWER(hits.eventInfo.eventLabel) = 'display ad'
		THEN 1
	ELSE 0
END) AS Display_Ads_Shown
,SUM(CASE
	WHEN LOWER(hits.eventInfo.eventAction) = 'show ad'
	AND LOWER(hits.eventInfo.eventCategory) = 'advertising'
	AND LOWER(hits.eventInfo.eventLabel) = 'sponsored listing'
		THEN 1
	ELSE 0
END) AS Sponsored_Listings_Shown
,CASE
WHEN REGEXP_MATCH(LOWER(trafficsource.campaign) r'^(c:b)')
	THEN 'SEM Brand'
WHEN REGEXP_MATCH(LOWER(trafficsource.campaign) r'^(c:nb)')
	THEN 'SEM Brand'
WHEN REGEXP_MATCH(LOWER(trafficsource.campaign) r'^(c:de)')
	THEN 'Display & Emerging'
WHEN REGEXP_MATCH(LOWER(trafficsource.campaign) r'^(c:eng)')
	THEN 'Digital Engagement'
WHEN REGEXP_MATCH(LOWER(trafficsource.campaign) r'^(c:ce)')
	THEN 'Consumer Email'
WHEN REGEXP_MATCH(LOWER(trafficsource.campaign) r'^(c:ae)')
	THEN 'Attorney Email'
WHEN REGEXP_MATCH(LOWER(trafficsource.campaign) r'^(c:p)')
	THEN 'Partners'	
...
OR LOWER(trafficSource.campaign) CONTAINS 'avvo_brand'
...
ELSE NULL
END) AS Northstar_Engagement_Flag
,SUM(CASE
WHEN LOWER(hits.eventInfo.eventAction) CONTAINS 'contact website'
...
THEN 1
ELSE 0
END) Messages_to_Advertisers
,SUM(CASE
WHEN LOWER(hits.eventInfo.eventAction) CONTAINS 'initiate mobile phone'
AND LOWER(hits.eventInfo.eventCategory) = 'advertising'
THEN 1
ELSE 0
END) Mobile_Calls_to_Advertisers
,SUM(CASE
WHEN LOWER(hits.eventInfo.eventAction) CONTAINS 'contact message'
AND LOWER(hits.eventInfo.eventCategory) = 'lawyer'
THEN 1
ELSE 0
END) Messages_to_NonAdvertisers
,SUM(CASE
WHEN LOWER(hits.eventInfo.eventAction) CONTAINS 'initiate mobile phone'
AND LOWER(hits.eventInfo.eventCategory) = 'lawyer'
THEN 1
ELSE 0
END) Mobile_Calls_to_NonAdvertisers
,SUM(CASE
WHEN LOWER(hits.eventInfo.eventAction) CONTAINS 'contact website'
THEN CAST(1 AS INTEGER)
ELSE CAST(0 AS INTEGER)
...
THEN hits.transaction.transactionid
ELSE NULL
END) Services_Purchases_Advisor
,EXACT_COUNT_DISTINCT(CASE
WHEN hits.eventInfo.eventCategory = 'ecommerce'
AND hits.eventInfo.eventAction = 'purchase avvo legal service'
...
THEN hits.transaction.transactionid
ELSE NULL
END) Services_Purchases_Document_Review
,EXACT_COUNT_DISTINCT(CASE
