...
else 'Paid Marketing' end channel
...
, d.year_month
, case 
	when a.ad_market_block_flag = 'Y' 
		then 'Block' 
	else 'Exclusive' 
end market_type
, a.ad_market_id
,sum(case 
		when v3.lpv_source = '' and v3.lpv_medium = '' and v3.lpv_campaign = '' and v3.lpv_content = '' 
			then 0 
...
from dm.ad_attribution_v3_all v3
join dm.date_dim d 
	on d.actual_date = v3.event_date
join dm.ad_market_dimension a 
	on a.ad_market_id = v3.ad_market_id
...
, case 
	when avg(total_rev) > avg(total_acv) 
		then (sum(acv)/avg(total_acv))*avg(total_rev)
	when channel_new NOT IN ('Direct', 'Referral', 'SEO Non-Brand', 'SEO Brand') and avg(total_rev) > avg(other_acv_new)
	...
	when avg(other_acv_new) < avg(total_rev) then (sum(acv)/avg(other_acv_new))*avg(other_acv_new) 
		else (sum(acv)/avg(other_acv_new))*avg(total_rev) 
end revenue_m2_new
, case 
	when avg(total_rev) > avg(total_acv) 
		then  1
	...
	when channel_new NOT IN ('Direct', 'Referral', 'SEO Non-Brand', 'SEO Brand') and avg(total_rev) <= avg(other_acv_new)
		then 3
	when avg(other_acv_new) < avg(total_rev) 
		then 4
	else 5 
end revenue_m2_new_test
from (
select 'Sponsored' inventory_type
, d.year_month
, a.market_type
