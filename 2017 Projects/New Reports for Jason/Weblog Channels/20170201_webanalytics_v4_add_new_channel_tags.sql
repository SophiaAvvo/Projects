...
/* duplication across e.g. ad market */
,deduped_webanalytics AS (
SELECT DISTINCT w.lpv_campaign
,w.lpv_source
,w.lpv_content
,w.lpv_medium
,w.lpv_referring_domain
,w.LPV_Page_Type
,w.session_id
,w.event_date
...
SELECT w.lpv_campaign
,w.lpv_source
,w.lpv_medium
,w.LPV_Page_Type
,CASE
wHEN w.LPV_Page_Type = 'Attorney_Directory_Launch'
THEN 'Find a Lawyer'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^Professional_')
THEN 'Lawyer Dashboard'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^(Attorney_Profile|Attorney_Review)')
THEN 'Lawyer Profile'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^(Attorney_Directory|Attorney_Search)')
THEN 'Lawyer SERP'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^Legal_Ask')
THEN 'Ask a Lawyer'
wHEN w.LPV_Page_Type = 'Legal_KB_Launch'
THEN 'Free Legal Advice'
wHEN w.LPV_Page_Type = 'Legal_Guides_Detail'
THEN 'Guide Detail'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^Legal_KB')
THEN 'KB SERP'
wHEN w.LPV_Page_Type = 'Legal_Answers_Detail'
THEN 'QA Detail'
wHEN w.LPV_Page_Type = 'Topics'
THEN 'Topics'
wHEN w.LPV_Page_Type = 'Homepage'
THEN 'Homepage'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^(LS-|SubCat|ALS )')
THEN 'LS'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^Advisor-')
THEN 'Advisor'
WHEN REGEXP_LIKE(w.LPV_Page_Type, '^SEM-')
THEN 'SEM'
ELSE 'Others'
...
,case  
WHEN STRLEFT(LOWER(w.lpv_campaign), 3) = 'c:b'
	THEN 'SEM Brand'
WHEN STRLEFT(LOWER(w.lpv_campaign), 4) = 'c:nb'
	THEN 'SEM Brand'
WHEN STRLEFT(LOWER(w.lpv_campaign), 4) = 'c:de'
	THEN 'Display & Emerging'
WHEN STRLEFT(LOWER(w.lpv_campaign), 5) = 'c:eng'
	THEN 'Digital Engagement'
WHEN STRLEFT(LOWER(w.lpv_campaign), 4) = 'c:ce'
	THEN 'Consumer Email'
WHEN STRLEFT(LOWER(w.lpv_campaign), 4) = 'c:ae'
	THEN 'Attorney Email'
WHEN STRLEFT(LOWER(w.lpv_campaign), 3) = 'c:p'
	THEN 'Partners'	  
...
,SUM(em.emails_and_messages) AS emails_and_messages_gross
,SUM(wb.website_clicks) AS website_clicks
FROM deduped_webanalytics w
 LEFT JOIN cnt_emails em
	ON em.session_id = w.session_id
	AND em.event_date = w.event_date
LEFT JOIN cnt_webcontacts wb
	ON wb.session_id = w.session_id
	AND wb.event_date = w.event_date
