...
AND referring_domain IS NOT NULL (redundant) */
AND page_type = 'Homepage'
	THEN 'SEO Brand'
/* finalized */
/*when source IN ('elocal', 'pbx', 'marchex') then 'Marchex/eLocal/pbx Calls'
 no results at this time; we do not track in weblog */
when (campaign in ('brand', 'Branded_Terms', 'legalbroad', 'Brand|RLSA') 
		or content IN ('brand', '["brand","brand"]')
		OR LOWER(campaign) LIKE '["brand"%'
		OR campaign LIKE 'brand,Branded_Terms%'
		OR campaign LIKE 'Branded_Terms|%')
...
AND page_type <> 'Homepage' redundant */
	THEN 'SEO Non-Brand'
WHEN (LOWER(campaign) LIKE 'legal_q_&_a_search' 
OR LOWER(campaign) LIKE '%pls|%'
OR LOWER(campaign) LIKE '%legalqa%'
OR LOWER(campaign) LIKE '%plsremarketing%' /*this also shows up for medium = display !! */
OR LOWER(campaign) LIKE '%advisorremarketing%'
OR campaign = 'pls')
AND (CASE 
WHEN LOWER(campaign) LIKE '%brand%' /* added 01/19 on mira's recommendation */
THEN 1
