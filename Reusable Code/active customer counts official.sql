...
-- before the UNION.
-- to get daily data, just use the part below the union All (expand the date range) and replace year_month with day
-- Current advertisers are those for which the expire datetime is greater than the current date according to the below logic,
-- meaning that the actual expire datetime in order_line_accumulation_fact will show as NULL or 1900-01-01 or as a future date.
-- Avvo pro is a different order line item; you will need to change the logic to look at product_line_id = 4 
--(you can see detail on this in the dm.product_line_dimension table)
