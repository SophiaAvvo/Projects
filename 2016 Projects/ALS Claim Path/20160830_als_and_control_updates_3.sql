...
,als.first_purchase_date
  ,COUNT(session_id) OVER(PARTITION BY t.persistent_session_id) total_sessions_count
  ,COUNT(event_date) OVER(PARTITION BY t.persistent_session_id) total_days_visited
  ,COUNT(event_date) OVER(PARTITION BY t.persistent_session_id, CASE WHEN t.event_date <= als.first_purchase_date THEN "Before Purchase" ELSE "After Purchase" END)
  ,ROW_NUMBER() OVER(PARTITION BY t.persistent_session_id ORDER BY event_date, session_id) Num 
FROM dm.traffic t
...
,COUNT(DISTINCT t.event_date) total_days_visited -- incorrect
,SUM(CASE
...
AND Num = 1
AND (eu.total_sessions_count >= 2 OR eu.is_als_customer = 1)
