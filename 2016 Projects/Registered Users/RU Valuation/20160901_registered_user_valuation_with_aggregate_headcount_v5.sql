/* replace with tmp_data_dm.sr_registered_user_activity_dates
with user_dates AS (
...
,count(DISTINCT ci.contact_item) as email_and_message_count
from email_prep ci
join dm.date_dim dt 
	on ci.event_date = dt.actual_date
	AND ci.user_id <> '-1'
JOIN tmp_data_dm.sr_registered_user_activity_dates ur 
...
ur.user_cohort_registration_month
,ur.user_action_month_begin
,count(DISTINCT ci.user_id) as emailer_count
from email_prep ci
join dm.date_dim dt 
	on ci.event_date = dt.actual_date
	AND ci.user_id <> '-1'
...
,ur.month_number
,ur.month_name
  , count(DISTINCT ur.user_account_id) as registered_users_count
from tmp_data_dm.sr_registered_user_activity_dates ur 
...
join dm.date_dim dt 
	on to_date(dt.actual_date)=to_date(q.created_at)
	AND approval_status_id in (1,2) -- updated
	AND q.created_by <> -1
JOIN tmp_data_dm.sr_registered_user_activity_dates ur 
	ON ur.user_account_id = q.created_by
	AND ur.user_action_month_begin = dt.month_begin_date
...
ur.user_cohort_registration_month
,ur.user_action_month_begin
, COUNT(DISTINCT pfrv.id) as approved_review_count
...
,UID_PID AS (
SELECT DISTINCT CAST(t.resolved_user_id AS INT) user_id
,t.persistent_session_id
FROM dm.traffic t
WHERE t.event_date >= '2015-09-01'
...
als_prep AS (
SELECT COALESCE(pv.user_id, CAST(up.user_id AS STRING)) user_id
-- ,pv.user_id pv_user_id
-- ,t.user_id t_user_id
,cast(regexp_extract(url, 'thank_you\/([0-9]+)', 1) as INT) order_id
,MIN(pv.event_date) event_date
from src.page_view pv
LEFT JOIN UID_PID up
	ON up.persistent_session_id = pv.persistent_session_id
where LOWER(page_type) LIKE '%thankyou%' 
	and event_date >= '2015-09-01'
GROUP BY 1,2
)
...
JOIN tmp_data_dm.sr_registered_user_activity_dates ur 
	ON ur.user_account_id = CAST(als.user_id AS INT)
	AND ur.user_action_month_begin = dt.month_begin_date
	-- AND ur.month_number <= 6
GROUP BY 1,2
...
ur.user_cohort_registration_month
,ur.user_action_month_begin
,COUNT(DISTINCT ur.user_account_id) AS als_customer_count
FROM als_prep als
	join dm.date_dim dt 
		on dt.actual_date = als.event_date
JOIN tmp_data_dm.sr_registered_user_activity_dates ur 
...
,rv2.reviewer_count
,als.als_purchase_count
,ac.als_customer_count
from cnt_ru uad 
...
left join cnt_emailer er
	on er.user_cohort_registration_month = uad.user_cohort_registration_month
	AND er.user_action_month_begin = uad.user_action_month_begin	
left join cnt_website wb 
	on wb.user_cohort_registration_month = uad.user_cohort_registration_month
	AND wb.user_action_month_begin = uad.user_action_month_begin
left join askers a 
...
left join als_customers ac
	on ac.user_cohort_registration_month = uad.user_cohort_registration_month
	AND ac.user_action_month_begin = uad.user_action_month_begin
