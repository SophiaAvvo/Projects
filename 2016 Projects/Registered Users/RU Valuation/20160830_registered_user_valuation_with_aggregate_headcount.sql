...
JOIN dm.date_dim dd
	on uad.user_account_register_datetime >= '2016-01-01'
	AND DD.actual_date BETWEEN to_date(uad.user_account_register_datetime) AND DATEADD(dd, uad.user_account_register_datetime, 180)
...
(
select
  month_begin_date AS cohort_registration_month
  ,
    ,rp.registration_path
	,rp.lawyer_vs_consumer
  , count(*) as num_users
...
SELECT x.*
,x.cnt_emailcontacts/x.num_users
,x.cnt_webcontacts/x.num_users
,x.cnt_visit/x.num_users
,x.num_questions/x.num_users w_avg_questions_per_ru
,x.num_reviews/x.num_users w_avg_reviews_per_ru
,x.num_askers/x.num_users %_users_who_asked_questions
,x.num_reviewers/