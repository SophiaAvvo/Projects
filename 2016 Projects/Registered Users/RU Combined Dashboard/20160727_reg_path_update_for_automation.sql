...
SELECT DISTINCT t.persistent_session_id ps_id
	...
	,from_unixtime(unix_timestamp(cast(ui.reg_time as timestamp) + interval 30 days), 'yyyy-MM-dd HH:mm:ss') WindowEnd
 from dm.traffic t
 JOIN dm.user_account_dimension ua
 ON ua.user_id = CAST(t.resolved_user_id aS INT)
...
,als_path AS (
	...
	FROM als_transactions a
		JOIN UID_2_PID up
			ON up.persistent_session_id = a.persistent_session_id
