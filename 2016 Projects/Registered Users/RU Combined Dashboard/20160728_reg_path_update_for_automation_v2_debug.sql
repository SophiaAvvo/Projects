...
JOIN UID_2_PID up
	...
	AND from_unixtime(ci.gmt_timestamp, 'yyyy-MM-dd HH:mm:ss') <= up.WindowEnd
...
JOIN UID_2_PID up
	...
	AND ci.created_at BETWEEN up.WindowStar t up.WindowEnd
left join PA3 pa
	ON pa.professional_id = ci.professional_id
...
,als_path AS (
	...
	FROM als_transactions a
		JOIN UID_2_PID up
			...
			AND a.transaction_time <= up.WindowEnd
