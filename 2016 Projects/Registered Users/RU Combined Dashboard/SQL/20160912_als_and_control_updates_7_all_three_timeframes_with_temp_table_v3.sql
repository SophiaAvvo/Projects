...
,eu.first_purchase_date
,eu.is_als_customer
,MIN(ep.event_date) OVER(PARTITION BY ep.persistent_session_id) AS FirstEmail
,MAX(ep.event_date) OVER(PARTITION BY ep.persistent_session_id) AS LastEmail
FROM email_prep1 ep
...
website_prep AS (
SELECT ci.persistent_session_id
,ci.render_instance_guid
,ci.event_date
,eu.first_purchase_date
,eu.is_als_customer
,MIN(ci.event_date) OVER(PARTITION BY ci.persistent_session_id) AS FirstWebsiteClick
,MAX(ci.event_date) OVER(PARTITION BY ci.persistent_session_id) AS LastWebsiteClick
from src.contact_impression ci
JOIN tmp_data_dm.sr_als_eligible_users eu
