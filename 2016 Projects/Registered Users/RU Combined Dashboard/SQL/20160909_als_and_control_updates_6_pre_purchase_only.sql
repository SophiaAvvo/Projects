...
SELECT t.persistent_session_id
,'Post-ALS-Launch-and-Pre-ALS-Purchase'
,COUNT(DISTINCT CASE
			WHEN eu.Is_ALS_Customer = 0
				THEN t.event_date
			WHEN eu.Is_ALS_Customer = 1 AND t.event_date <= eu.first_purchase_date
				THEN t.event_date
			ELSE NULL
		END) days_visited
,COUNT(DISTINCT CASE
...
,eu.resolved_user_id
,eu.session_check
,eu.event_date AS first_postlaunch_visit_date
,t.lpv_page_type
,t.lpv_medium
...
AND Num = 1
AND (eu.session_check >=2 OR eu.is_als_customer = 1)
AND (CASE WHEN eu.is_als_customer = 1 THEN 1 WHEN STRRIGHT(eu.persistent_session_id, 1) IN ('1', 'a', '7', '3', 'e') THEN 1 ELSE 0 END) = 1
...
LEFT JOIN src.page_view pv -- okay to join to pageview this way because all our dimensions are elsewhere and we are aggregating all of this
	...
	AND (eu.first_purchase_date >= pv.event_date OR eu.is_als_customer = 0)
left join dm.specialty_dimension sd 
...
from email_prep ci
JOIN eligible_users eu
ON eu.resolved_user_id = ci.user_id
AND eu.Num = 1
AND (eu.session_check >=2 OR eu.is_als_customer = 1)
WHERE (eu.first_purchase_date >= ci.event_date OR eu.is_als_customer = 0)
group by 1,2-- ,3
...
AND ci.event_date >= '2016-02-08'
	AND (eu.first_purchase_date >= ci.event_date OR eu.is_als_customer = 0)
group by 1,2--,3
...
WHERE ci.created_at >= '2016-02-08'
	AND (eu.first_purchase_date >= ci.created_at OR eu.is_als_customer = 0) 	
...
from review2 r
WHERE mindaysdiff = 1 -- the review is attributed to the most recently-used PID
group by 1,2--,3
...
JOIN eligible_users eu
	...
	where q.approval_status_id in (1,2)
	   AND (q.created_by <> 1 OR q.updated_by <> 1)
		...
		AND (eu.first_purchase_date >= q.created_at OR eu.is_als_customer = 0) 		   
...
FROM user_visits uv
	JOIN users_detail ud
		ON ud.persistent_session_id = uv.persistent_session_id
AND uv.Timeframe = 'Post-ALS-Launch-and-Pre-ALS-Purchase'		
