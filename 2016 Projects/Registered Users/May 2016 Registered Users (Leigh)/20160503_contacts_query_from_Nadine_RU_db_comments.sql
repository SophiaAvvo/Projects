...
, d.year_month
, min(order_ln_begin_use_date) as order_ln_begin_use_date
, max(order_ln_cncl_date ) as order_ln_cncl_date
from dm.order_ln_accum_fact olaf
join dm.date_dim d on d.actual_date = order_ln_begin_use_date
where d.year_month >= 201505
and olaf.product_ln_key in (2,7,4) -- 2 = Display Med Rectangle, 7 = SL, 4 = Pro
group by 1,2,3
) 
...
left join b on b.profnl_id  = ci.professional_id and d.year_month = b.year_month and  b.order_ln_begin_use_date <= ci.event_date
