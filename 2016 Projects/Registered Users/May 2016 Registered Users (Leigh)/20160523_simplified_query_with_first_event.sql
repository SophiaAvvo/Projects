...
,CASE
	...
	ELSE 'Other'
END ParentPA
,min(from_utc_timestamp(ci.`timestamp`, 'pst')) FirstActionTime
--, max(ci.created_at) as LastActionTime
from src.contact_impression ci
JOIN tmp_data_dm.sr_ru_user_id_and_reg_date ui
	ON ui.user_id = CAST(ci.user_id AS INT)
	AND from_utc_timestamp(ci.`timestamp`, 'pst') < /*BETWEEN from_unixtime(unix_timestamp(cast(ui.reg_time as timestamp) - interval 1 days), 'yyyy-MM-dd HH:mm:ss') AND */ui.reg_time
AND ui.reg_time < '2016-05-01'
