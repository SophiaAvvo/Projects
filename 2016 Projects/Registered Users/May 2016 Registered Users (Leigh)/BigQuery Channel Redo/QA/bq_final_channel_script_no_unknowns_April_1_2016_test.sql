SELECT y.user_id AS user_id
    ...
    ,x.persistent_session_id AS psid_main
    ,x.VisitStartTime AS vst_main
...
WHEN (
			  ...
			  WHEN LOWER(trafficSource.campaign) CONTAINS 'display'
				THEN 'Display - AMM'
                          ...
                          ELSE '(Other)'
                        END AS Channel
                      ,MAX(IF(hits.customDimensions.index=61,CASE WHEN hits.customDimensions.value = '(unknown)' THEN NULL ELSE hits.customDimensions.value END, NULL)) WITHIN hits as Persistent_Session_ID
                       FROM [75615261.ga_sessions_20160401]
