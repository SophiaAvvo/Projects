...
ci.created_by user_id
	...
	,CASE
		WHEN sd.parent_specialty_name IN ('Family', 'Business', 'Immigration', 'Real Estate', 'Estate Planning')
...
JOIN tmp_data_dm.sr_ru_user_id_and_reg_date ui
	...
	AND ci.created_at <= from_unixtime(unix_timestamp(cast(ui.reg_time as timestamp) + interval 3 days), 'yyyy-MM-dd HH:mm:ss')
...
JOIN tmp_data_dm.sr_ru_user_id_and_reg_date ui
	...
	AND from_unixtime(ci.gmt_timestamp, 'yyyy-MM-dd HH:mm:ss') <= from_unixtime(unix_timestamp(cast(ui.reg_time as timestamp) + interval 3 days), 'yyyy-MM-dd HH:mm:ss')
AND ui.reg_time < '2016-05-01'
...
(
	...
	JOIN tmp_data_dm.sr_ru_user_id_and_reg_date ui
		...
		AND q.created_at <= from_unixtime(unix_timestamp(cast(ui.reg_time as timestamp) + interval 3 days), 'yyyy-MM-dd HH:mm:ss')
