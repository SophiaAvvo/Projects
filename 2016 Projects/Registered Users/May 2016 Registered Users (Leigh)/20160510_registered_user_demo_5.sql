...
WITH demo AS (select user_account_id user_id
	...
	,dd.yearmonth
	,CASE 
		WHEN ua.user_account_registration_datetime < dd.month_begin_date
...
daily_traffic AS (
	...
	SELECT up.user_id
		...
		,SUM(CASE
			...
			END) MobileDevicePVs
		,SUM(CASE
				WHEN t.lpv_device_category_id IN (4,5)
					THEN  cast(regexp_extract(page_type_summary, '"Total": ([0-9]+)', 1) as int)
				ELSE 0
			END) EntertainmentSystemPVs
		,SUM(CASE
				WHEN t.lpv_device_category_id = 2
					THEN cast(regexp_extract(page_type_summary, '"Total": ([0-9]+)', 1) as int)
				ELSE 0
			END) DesktopPVs
		,SUM(CASE
		...
		,SUM(CASE
				WHEN t.lpv_device_category_id = 8
					THEN  cast(regexp_extract(page_type_summary, '"Total": ([0-9]+)', 1) as int)
				ELSE 0
			END) OtherDevicePVs
		,SUM(CASE
