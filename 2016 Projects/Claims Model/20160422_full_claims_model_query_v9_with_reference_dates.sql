...
SELECT pfad.PROFESSIONAL_ID
  ...
  FROM (SELECT pd.*
        ...
        FROM (SELECT pf.PROFESSIONAL_ID
					 ...
					 ,CASE
						WHEN pf.claim_date IS NULL
							then to_date(now() - interval 1 day)
						ELSE to_date(to_utc_timestamp(pf.claim_date, 'pdt') - interval 1 day) 
						END AS reference_date_prior_day
					,CASE
						WHEN pf.claim_date IS NULL
							then to_date(now() - interval 31 days)
						ELSE to_date(to_utc_timestamp(pf.claim_date, 'pdt') - interval 31 days) 
...
select dto.YEAR_MONTH
                             ...
                             , SUM(case when pln.PRODUCT_LINE_ITEM_NAME = 'Sponsored Listing' and olaf.order_line_indicator_id  in (1,2,3) and olaf.BLOCK_COUNT = 0 then 1
