...
SELECT pfad.PROFESSIONAL_ID
  ...
  FROM (SELECT pd.*
        ...
        FROM (SELECT pf.PROFESSIONAL_ID
					 ...
					 ,from_unixtime(unix_timestamp(cast(professional_claim_date as timestamp) - interval 1 days), 'yyyy-MM-dd HH:mm:ss') AS reference_datetime_prior_day
					,from_unixtime(unix_timestamp(cast(professional_claim_date as timestamp) - interval 31 days), 'yyyy-MM-dd HH:mm:ss') AS reference_datetime_prior_month	
					 ,CASE
...
,p.reference_datetime_prior_day
,p.reference_datetime_prior_month
		,p.is_claim
