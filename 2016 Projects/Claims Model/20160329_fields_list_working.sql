...
SELECT pf.*
      ...
      ,r.PercentRecommended
       ...
       ,CASE
          WHEN pf.emaildomain LIKE '%@gmail%'
            THEN 1
          WHEN pf.emaildomain LIKE '%@yahoo%'
            THEN 1
          WHEN pf.emaildomain LIKE '%@aol%'
            THEN 1
          WHEN pf.emaildomain LIKE '%@msn%'
            THEN 1
          WHEN pf.emaildomain LIKE '%@hotmail%'
            THEN 1
          ELSE 0
...
FROM deduped_pfad pf
  ...
  LEFT JOIN (SELECT pf.emaildomain
              ,COUNT(DISTINCT pf.professional_id) DomainSize
              FROM deduped_pfad pf
