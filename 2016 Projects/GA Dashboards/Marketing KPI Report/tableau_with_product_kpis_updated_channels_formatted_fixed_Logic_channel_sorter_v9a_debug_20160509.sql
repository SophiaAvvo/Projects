...
END) AS  ClicksToLawyerWebsite
			...
			,SUM(CASE
			    ...
			    END) UncontestedDivorcePackagePurchases
                       FROM TABLE_DATE_RANGE ([75615261.ga_sessions_], TIMESTAMP('2015-10-18'), TIMESTAMP('2015-10-19'))--CURRENT_TIMESTAMP())
...
(SELECT                date AS Date
			...
			,SUM(CASE
				...
				ELSE 0
			    END) UncontestedDivorcePackagePurchases   
                       FROM TABLE_DATE_RANGE ([75615261.ga_sessions_], TIMESTAMP('2014-05-27'),TIMESTAMP('2014-5-28'))--TIMESTAMP('2015-06-06'))
                       WHERE (CASE WHEN LOWER(trafficSource.campaign) CONTAINS 'network' OR LOWER(trafficSource.campaign) CONTAINS 'sgt' THEN 1 ELSE 0 END) = 0   
                      GROUP BY 1,2
                      )
			  ...
			  WHEN trafficSource.campaign CONTAINS 'display'
				THEN 'Display - AMM'
                       ...
                       ,COUNT(CASE
                                ...
                                ELSE NULL
                              END) NumberOfRegistrations
                       ,0 EmploymentLaborAdvicePurchases
			...
			,0 UncontestedDivorcePackagePurchases       
                       FROM TABLE_DATE_RANGE ([75615261.ga_sessions_], TIMESTAMP('2014-02-09'), TIMESTAMP('2014-02-10'))  -- ,TIMESTAMP('2014-05-26'))
                       WHERE (CASE WHEN LOWER(trafficSource.campaign) CONTAINS 'network' OR LOWER(trafficSource.campaign) CONTAINS 'sgt' THEN 1 ELSE 0 END) = 0
