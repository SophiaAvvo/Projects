...
,COUNT(CASE
WHEN LOWER(hits.eventInfo.eventAction) = 'new registration' -- totally dead wrong
AND LOWER(hits.eventInfo.eventCategory) = 'users'
THEN CAST(1 AS INTEGER)
ELSE CAST(0 AS INTEGER)
END) NumberOfRegistrations1
,EXACT_COUNT_DISTINCT(CASE
...
THEN CONCAT(fullVisitorId,string(VisitId))
ELSE NULL
END) NumberOfRegistrations2
,SUM(CASE
WHEN hits.customMetrics.index = 3
THEN 1
ELSE 0
END) NumberOfRegistrations3
,EXACT_COUNT_DISTINCT(CASE
...
,SUM(CASE
		WHEN hits.customMetrics.index IN (10, 11)
			THEN CAST(1 AS INTEGER)
		ELSE CAST(0 AS INTEGER)
	END) EmailSubscriptions1
,EXACT_COUNT_DISTINCT(CASE
		WHEN hits.customMetrics.index = 11
			THEN CONCAT(fullVisitorId,string(VisitId))
		ELSE NULL
	END) OffsiteEmailSubscriptions2
