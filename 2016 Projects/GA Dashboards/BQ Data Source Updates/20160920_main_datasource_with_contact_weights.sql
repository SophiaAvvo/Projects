...
Date AS Date
	...
	,Non_Phone_Contacts_Weekly_Channel/Non_Phone_Contacts_Weekly_Total Phone_Attribution_Weights_Daily
	,Non_Phone_Contacts_Quarterly_Channel/Non_Phone_Contacts_Quarterly_Total Phone_Attribution_Weights_Daily	
    ...
    ,als_advice_bankruptcy_and_debt AS als_advice_bankruptcy_and_debt
    ,als_advice_business AS als_advice_business
    ,als_advice_criminal_defense AS als_advice_criminal_defense
    ,als_advice_divorce_and_separation AS als_advice_divorce_and_separation
	,als_advice_employment_and_labor AS als_advice_employment_and_labor
	,als_advice_estate_planning AS als_advice_estate_planning
	,als_advice_family AS als_advice_family
    ,als_advice_immigration AS als_advice_immigration
    ,als_advice_landlord_and_tenant AS als_advice_landlord_and_tenant
	,als_advice_real_estate AS als_advice_real_estate
    ,als_advice_15_minute AS als_advice_15_minute
    ,als_doc_review_family_green_card AS als_doc_review_family_green_card
	,als_doc_review_app_us_citizenship AS als_doc_review_app_us_citizenship
	,als_doc_review_business_contract AS als_doc_review_business_contract
    ...
    ,als_doc_review_contractor_agreement AS als_doc_review_contractor_agreement
	,als_doc_review_employment_contract AS als_doc_review_employment_contract
	...
	,als_doc_review_non_disclosure_agreement AS als_doc_review_non_disclosure_agreement
	,als_doc_review_parenting_plan AS als_doc_review_parenting_plan
	,als_doc_review_power_of_attorney AS als_doc_review_power_of_attorney
	,als_doc_review_prenuptial_agreement AS als_doc_review_prenuptial_agreement
	...
	,als_doc_review_separation_agreement AS als_doc_review_separation_agreement
	,als_doc_review_vendor_agreement AS als_doc_review_vendor_agreement		
	...
	,als_doc_review_demand_letter_for_payment AS als_doc_review_demand_letter_for_payment
	,als_doc_review_demand_letter_for_breach_of_contract AS als_doc_review_demand_letter_for_breach_of_contract
	,als_offline_family_green_card AS als_offline_family_green_card
	,als_offline_apply_for_us_citizenship AS als_offline_apply_for_us_citizenship
	,als_offline_create_a_business_contract AS als_offline_create_a_business_contract
	,als_offline_create_a_parenting_plan AS als_offline_create_a_parenting_plan
	...
	,als_offline_create_a_demand_letter AS als_offline_create_a_demand_letter
    , als_offline_file_for_uncontested_divorce AS als_offline_file_for_uncontested_divorce
	,als_offline_form_c_or_s_corp AS als_offline_form_c_or_s_corp
	,als_offline_start_a_single_member_llc AS als_offline_start_a_single_member_llc
	,MarketingFees AS MarketingFees
...
FROM (SELECT Date AS Date
	...
	,SUM(Email_and_Web_Contacts) OVER(PARTITION BY YEAR(Date), QUARTER(Date), Channel) AS Non_Phone_Contacts_Quarterly_Channel
	,SUM(Email_and_Web_Contacts) OVER(PARTITION BY YEAR(Date), QUARTER(Date)) AS Non_Phone_Contacts_Quarterly_Total
	,SUM(Email_and_Web_Contacts) OVER(PARTITION BY YEAR(Date), WEEK(Date), Channel) AS Non_Phone_Contacts_Weekly_Channel
	,SUM(Email_and_Web_Contacts) OVER(PARTITION BY YEAR(Date), WEEK(Date)) AS Non_Phone_Contacts_Weekly_Total
    ,ClicksToLawyerWebsite AS ClicksToLawyerWebsite
    ,NumberQuestionsAsked AS NumberQuestionsAsked
    ,NumberQuestionsAnswered AS NumberQuestionsAnswered
    ,EmailSentToLawyers AS EmailSentToLawyers
	...
	,als_doc_review_termination_letter AS als_doc_review_termination_letter
	,als_doc_review_employment_offer_letter AS als_doc_review_employment_offer_letter
	,als_doc_review_demand_letter_payment AS als_doc_review_demand_letter_for_payment
	,als_doc_review_demand_letter_breach_of_contract AS als_doc_review_demand_letter_for_breach_of_contract
	,als_offline_family_green_card AS als_offline_family_green_card
	...
	,TotalAdvisorPurchases*10.0
		+(als_doc_review_power_of_attorney 
			+ als_doc_review_living_will
			+ als_doc_review_demand_letter_payment)*30.0
		+ (FamilyGreenCardReviewPurchases
			...
			+ als_doc_review_demand_letter_breach_of_contract*45.0
		...
		+ (als_offline_create_a_residential_lease_agreement
		+ als_offline_create_a_demand_letter)*100.0
...
END) AS  ClicksToLawyerWebsite
			...
			,SUM(CASE
					WHEN hits.eventInfo.eventCategory = 'ecommerce'
					AND hits.eCommerceAction.action_type = '6'
					AND hits.product.v2Productname = 'Document review: Demand letter for payment'
						THEN hits.Product.ProductQuantity
					ELSE CAST(0 AS INTEGER)
				END) als_doc_review_demand_letter_payment
			,SUM(CASE
					WHEN hits.eventInfo.eventCategory = 'ecommerce'
					AND hits.eCommerceAction.action_type = '6'
					AND hits.product.v2Productname = 'Document review: Demand letter for breach of contract'
						THEN hits.Product.ProductQuantity
					ELSE CAST(0 AS INTEGER)
				END) als_doc_review_demand_letter_breach_of_contract
			,SUM(CASE
					WHEN hits.eventInfo.eventCategory = 'ecommerce'
					AND hits.eCommerceAction.action_type = '6'
			...
			,SUM(CASE
					WHEN hits.eventInfo.eventCategory = 'ecommerce'
					AND hits.eCommerceAction.action_type = '6'
					AND hits.product.v2Productname = 'Create a demand letter'
						THEN hits.Product.ProductQuantity
					ELSE CAST(0 AS INTEGER)
				END) als_offline_create_a_demand_letter
...
(SELECT                date AS Date
			...
			,CAST(0 AS INTEGER) als_doc_review_demand_letter_payment
			,CAST(0 AS INTEGER) als_doc_review_demand_letter_breach_of_contract
			...
			,CAST(0 AS INTEGER) als_offline_create_a_demand_letter
			  ...
			  WHEN trafficSource.campaign CONTAINS 'display'
				THEN 'Display - AMM'
                        ...
                        END AS Channel
                        ,COUNT(totals.visits) AS Sessions
						,CAST(0 AS INTEGER) AS Email_and_Web_Contacts
                       ,SUM(CASE
                                WHEN LOWER(hits.eventInfo.eventAction) CONTAINS 'contact website'
                                  THEN CAST(1 AS INTEGER)
                                ELSE CAST(0 AS INTEGER)
