...
FROM (
	...
	CROSS JOIN (SELECT DISTINCT actual_date
				...
				,d.month_begin_date
              FROM dm.date_dim d
              WHERE d.actual_date BETWEEN '2015-01-01' AND to_date(now())
...
select DISTINCT o.professional_id
            		,o.order_line_begin_date ad_start_date
					,CASE
...
JOIN (SELECT DISTINCT actual_date
				-- ,d.year_month
				-- ,d.month_begin_date
              FROM dm.date_dim d
			WHERE d.actual_date BETWEEN '2015-01-01' AND to_date(now())
	) cal
			    ON  cal.actual_date >= pd.source_system_begin_date
