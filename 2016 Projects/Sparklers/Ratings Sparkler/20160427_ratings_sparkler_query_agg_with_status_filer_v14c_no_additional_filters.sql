...
INVALIDATE METADATA dm.historical_ad_customer_professional_map;
INVALIDATE METADATA src.barrister_licensing_authority_status;
INVALIDATE METADATA src.barrister_scoring_category;
INVALIDATE METADATA src.barrister_scoring_category_attribute; 
INVALIDATE METADATA src.history_barrister_professional_scoring_log;
INVALIDATE METADATA src.nrt_subscription;

Sarbendu says that the "practicing" filter is the same as Jeannie's "licensed & authorized to practice"
*/
...
/*JOIN src.barrister_licensing_authority_status st
	ON st.id = CAST(regexp_extract(url, '-([0-9]+)', 1) AS INT)
	AND st.license_status_type_id = 2*/
	WHERE event_date >= '2015-05-01'
  and page_type = 'Attorney_Profile'
  -- AND CAST(regexp_extract(url, '-([0-9]+)', 1) AS INT) < 10000
GROUP BY 1,2,3
-- ORDER BY 4 DESC
  ) a
GROUP BY 1,2
...
select
  ...
  on opsl.scoring_category_attribute_id=osca.id
    ...
    -- AND opsl.professional_id < 10000
