...
(
		...
		( */
				select -- dt.year_month
					...
					, ci.professional_id
					, count(CASE
								WHEN ci.contact_type = 'website'
									THEN ci.session_id
								ELSE NULL
							END) as website_visits
					, count(CASE
								WHEN ci.contact_type = 'phone'
									THEN ci.session_id
								ELSE NULL
							END) as phone_contacts
					, count(CASE
								WHEN ci.contact_type = 'email'
									THEN ci.session_id
								ELSE NULL
							END) as email_contacts
				from src.contact_impression ci
				join DM.DATE_DIM d 
				  on d.actual_date = ci.event_date
				AND d.actual_date >= '2015-01-01'
...
FROM (
        ...
        AND (cal.actual_date <= ad_end_date OR ad_end_date IS NULL)
        ) ad2
CROSS JOIN (SELECT DISTINCT actual_date
              FROM dm.date_dim d
              WHERE d.actual_date BETWEEN '2000-01-01' AND to_date(now())
