SELECT y.u_user_id
,y.traffic_first_visit_date
,x.ChannelDate
,x.FirstChannel
,x.persistent_session_id
...
,CASE
			  ...
			  WHEN LOWER(trafficSource.campaign) CONTAINS 'display'
				THEN 'Display - AMM'
                        ...
                        END AS Channel
                      ,MAX(IF(hits.customDimensions.index=61,hits.customDimensions.value, NULL)) WITHIN hits as Persistent_Session_ID
                       FROM TABLE_DATE_RANGE([75615261.ga_sessions_], TIMESTAMP('2015-05-27'),TIMESTAMP('2015-06-06'))
...
SELECT VisitStartTime
			  ...
			  WHEN trafficSource.campaign CONTAINS 'display'
				THEN 'Display - AMM'
                        ...
                        END AS Channel
                      ,MAX(IF(hits.customDimensions.index=61,hits.customDimensions.value, NULL)) WITHIN hits as Persistent_Session_ID
                       FROM TABLE_DATE_RANGE([75615261.ga_sessions_], TIMESTAMP('2014-05-09'),TIMESTAMP('2014-05-26'))
...
JOIN (SELECT u_user_id
		,u_persistent_session_id
		,Traffic_First_Visit_Date
		,f_FirstVisitTime
		,FROM Registered_Users_May_2016.UID_PID_first_visit_time) y
