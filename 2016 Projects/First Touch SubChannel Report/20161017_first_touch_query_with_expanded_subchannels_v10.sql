...
,UID_add AS (
SELECT DISTINCT ch.persistent_session_id
,ch.user_id
,event_date
,MIN(event_date) OVER(PARTITION BY user_id) first_visit_date
FROM dm.traffic t
...
,SUM(c.Is_Paid_Fb_RU) OVER(PARTITION BY c.persistent_session_id) Is_Paid_Fb_RU
,SUM(c.Is_Paid_Fb_ALS) OVER(PARTITION BY c.persistent_session_id) Is_Paid_Fb_ALS
...
,AVG(CASE
		WHEN fv.first_visit_timestamp < oas.created_at
			THEN DATEDIFF(oas.created_at, w.event_date)
		ELSE NULL
	END) avg_days_to_purchase_all
,AVG(CASE 
		WHEN op.name LIKE '%advice session%' AND fv.first_visit_timestamp < oas.created_at
			THEN DATEDIFF(oas.created_at, w.event_date)
		ELSE NULL
	END) avg_days_to_purchase_advisor
,AVG(CASE 
		WHEN op.name LIKE '%review%' AND fv.first_visit_timestamp < oas.created_at
			THEN DATEDIFF(oas.created_at, w.event_date)
		ELSE NULL
...
,AVG(CASE 
		...
		WHEN fv.first_visit_timestamp >= oas.created_at
			THEN NULL
		ELSE DATEDIFF(oas.created_at, w.event_date)
...
,SUM(CASE 
		...
		WHEN op.name LIKE '%advice session%'
			THEN 0
		WHEN op.name IS NOT NULL AND fv.first_visit_timestamp < oas.created_at
			THEN 1
		ELSE 0
