/* Channels has been replaced with tmp_data_dm.sr_paid_channels
...
/* Now we create the self-join that allows us to append the correct user_id to each PID *and* get additional PIDs that we haven't yet grabbed.
...
Note that the COALESCE is necessary so that we get those additional PIDs from the self-join, which aren't present in the first pull
due to not being affiliated with a paid channel.  */

,

UID_add AS (
  SELECT DISTINCT COALESCE(t2.persistent_session_id, t.persistent_session_id) persistent_session_id
,t.user_id
,CASE
...
,CASE
	...
	ELSE 0
END Is_RU
-- ,SUM(t.total_sessions) OVER(PARTITION BY CASE WHEN t.user_id IS NULL THEN t.persistent_session_id ELSE t.user_id END) total_sessions
-- ,COUNT(t.persistent_session_id) OVER(PARTITION BY CASE WHEN t.user_id IS NULL THEN t.persistent_session_id ELSE t.user_id END) PID_count
,rp.registration_path
,rp.registration_parent_pa
,rp.registration_parent_pa_group
,rp.lawyer_vs_consumer
,rp.user_account_register_datetime
-- ,MIN(event_date) min_date
FROM traffic t
	LEFT JOIN traffic t2
		ON t2.user_id = t.user_id	
	LEFT JOIN dm.rpt_user_type_and_registration_path rp
		ON rp.user_id = CAST(t.user_id AS INT)
WHERE t.persistent_session_id IN (SELECT DISTINCT persistent_session_id
		FROM tmp_data_dm.sr_paid_channels)
-- LIMIT 1000;
...
,ua.registration_path
,ua.registration_parent_pa
,ua.registration_parent_pa_group
,ua.lawyer_vs_consumer
,ua.user_account_register_datetime
FROM (SELECT *
		FROM tmp_data_dm.sr_paid_channels ch
...
questions AS (select 
   CAST(q.created_by AS STRING) AS user_id
...
,fv.Channel 
,fv.Is_RU
,fv.registration_path
,fv.registration_parent_pa
,fv.registration_parent_pa_group
,fv.lawyer_vs_consumer
,CASE
	WHEN fv.Is_RU = 1
	AND fv.user_account_register_datetime >= fv.first_visit_timestamp
		THEN 1
	ELSE 0
END Is_RU_Conversion
...
,q.num_questions
FROM channel_sort fv
JOIN (SELECT w.persistent_session_id
		,w.`timestamp`
		,CASE
	...
	AND w.url LIKE '%logged_in=true%'
		...
		,MAX(w.source) source
		,MAX(w.medium) medium
		,MAX(w.campaign) campaign
		FROM src.weblog w
		WHERE w.event_type IN('page_view', 'service_session_payment')
		GROUP BY 1,2,3,4
		) w
