/* deduped_pfad gets all professional information and cleans and dedupes it
georank assigns each geo area (usually city/state) to a number based on Nika's spreadsheet "Geo Table" listed in final results.  Some of the names were mangled so I edited the code to try and fix that */
...
,acv AS (
SELECT professional_id
,SUM(total_attributed_value) AS total_acv
...
FROM deduped_pfad gt
	...
	LEFT JOIN acv cv
		ON cv.professional_id = gt.professional_id
