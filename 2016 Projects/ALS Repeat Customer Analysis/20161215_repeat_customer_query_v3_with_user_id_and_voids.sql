...
FROM src.weblog w
JOIN als_transactions t -- inner join ensures that persistent_session_id and advice_session_id are 1:1
...
SELECT gp.advice_session_id
	...
	,COALESCE(oas.client_email_address, CAST(t.user_id AS VARCHAR), CAST(pv.persistent_session_id AS VARCHAR), CAST(gp.advice_session_id AS VARCHAR)) AS client_identifier
	,CASE
	...
	END IsOffline
	,CASE
		WHEN v.advice_session_id IS NULL
			THEN 1
		ELSE 0
	END AS IsSuccessfulTransaction
	,CASE
		WHEN pac.name LIKE "%advice session%"
			THEN 'Advice Session'
...
LEFT JOIN traffic t
	ON t.persistent_session_id = pv.persistent_session_id
LEFT JOIN voids v
	ON v.advice_session_id = gp.advice_session_id
LEFT JOIN first_channel_2 fc
	...
	--AND p.IsFirst = 1 -- only append to customer's first purchases (otherwise we get different values for subsequent persistent session IDs)
...
SELECT advice_session_id
	...
	,CASE
		WHEN first_advice_session_id = advice_session_id
			THEN 'New Customer Purchase'
		ELSE 'Repeat Purchase'
	END AS New_Or_Repeat
	,CASE
		WHEN customer_first_visit_date = first_visit_timestamp -- this is how we get rid of invalid first-visit channels for users with multiple PIDs
			THEN First_Visit_Channel
		ELSE NULL
	END AS First_Visit_Channel
	,CASE
		WHEN customer_first_visit_date = first_visit_timestamp -- for filtering the view on the first-touch channel
			THEN 'Use for First Visit Channel'
		ELSE 'Filter from First Visit Channel'
	END First_Visit_Filter
	,customer_first_purchase_date
			,customer_first_visit_date
			,first_advice_session_id
			,client_purchase_total
	FROM (SELECT *
