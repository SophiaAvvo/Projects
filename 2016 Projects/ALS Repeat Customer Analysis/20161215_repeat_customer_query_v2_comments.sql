...
-- now we can append the first channel for that client
SELECT p.*
,fc.First_Visit_Channel
,MIN(fc.first_visit_timestamp) OVER(PARTITION BY client_identifier) AS customer_first_visit_date
FROM penultimate p
LEFT JOIN first_channel_2 fc
	ON fc.persistent_session_id = p.persistent_session_id
