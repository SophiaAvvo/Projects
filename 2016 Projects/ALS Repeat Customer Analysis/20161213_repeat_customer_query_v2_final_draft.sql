...
,ROW_NUMBER() OVER(PARTITION BY advice_session_id ORDER BY CASE WHEN medium IS NOT NULL THEN 1 ELSE 2 END) AS DupeChecker -- because we occasionally see people who appear to come through both organic and a paid channel
...
SELECT oas.id AS advice_session_id
	...
	,CASE
		WHEN dcp.first_advice_session_id = oas.id
			THEN 1
		ELSE 0
	END IsFirst
	,CASE
...
LEFT JOIN purchase_visit_channel pv
	ON oas.id = CAST(pv.advice_session_id AS INT)
...
SELECT *
,ROW_NUMBER() OVER(PARTITION BY client_email_address, purchase_time_pst ORDER BY First_Visit_Channel)
