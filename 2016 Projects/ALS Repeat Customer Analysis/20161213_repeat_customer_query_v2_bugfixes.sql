...
SELECT gp.advice_session_id
	...
	,CASE
		WHEN dcp.first_advice_session_id = gp.advice_session_id
			THEN 1
		ELSE 0
	END IsFirst
	,CASE
		...
		ELSE 0
	END IsRepeat
	,MIN(COALESCE(gp.purchase_date_pst, oas.created_at_pst)) OVER(PARTITION BY COALESCE(oas.client_email_address, CAST(pv.persistent_session_id AS VARCHAR)) AS customer_first_purchase_date
